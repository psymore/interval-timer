...
// ── Mini window ───────────────────────────────────────────────
// Also used by the "mini:reset-bounds" IPC handler below — recomputed
// (not cached) each time so it still lands correctly if the primary
// display's work area changed since the window was first created.
function getDefaultMiniBounds() {
  const { width: sw } = screen.getPrimaryDisplay().workAreaSize;
  return { x: sw - 300, y: 20, width: 240, height: 180 };
}
...
export function registerWindowIpc() {
  ...
  // Manual resize drag (js/mini.js) — frameless windows on this Electron
  // version get no native edge/corner resize hit-testing (Chromium only
  // added that for frame:false windows in v42; this app is on v41), so
  // the renderer computes the new bounds itself from mouse deltas and the
  // main process just applies them.
  ipcMain.handle("mini:resize-start", () => {
    if (!miniWindow || miniWindow.isDestroyed()) return null;
    const [minWidth, minHeight] = miniWindow.getMinimumSize();
    return { ...miniWindow.getBounds(), minWidth, minHeight };
  });

  ipcMain.on("mini:resize-move", (_event, bounds) => {
  ...
  ipcMain.on("mini:reset-bounds", () => {
    if (miniWindow && !miniWindow.isDestroyed()) {
      miniWindow.setBounds(getDefaultMiniBounds());
    }
  });
}
